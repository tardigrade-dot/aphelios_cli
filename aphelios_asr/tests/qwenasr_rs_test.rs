...
async fn vad_and_qwenasr() -> Result<()> {
    ...
    for au in audio_li {
        ...
        let items = qwenasr::qwen3asr_with_vad(Some(ASR_MODEL_DIR), Some(ALIGNER_MODEL_DIR), Some("/Volumes/sw/onnx_models/silero-vad/onnx"), &au.path_str, &au.language_str, Some(context)).await?;
...
fn qwen3asr_simple_test() -> Result<()> {
    ...
    qwenasr::qwen3asr_simple(Some(&qwen3asr_model), Some(&aligner_model), &input, language).context("qwen3asr_simple error")?;
