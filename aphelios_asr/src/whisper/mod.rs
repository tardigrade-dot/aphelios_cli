                ...
                for &token in dr.tokens.iter() {
                    ...
                    if token > self.no_timestamps_token {
                        ...
                        if !tokens_to_decode.is_empty() {
                            ...
                            debug!("tokens is empty, {} - {}", prev_timestamp_s, timestamp_s);
                        }
...
pub fn run_whisper_with_segments(
    ...
    let pcm_data = if sample_rate != m::SAMPLE_RATE as u32 {
        ...
        let resampled = resampler.resample_mono(&input_buffer, m::SAMPLE_RATE as u32)?;
        resampled.samples
    } else {
        pcm_data
    };
    ...
    for batch in segments {
        ...
        let start_sample = (padded_start * m::SAMPLE_RATE as f64) as usize;
        let end_sample = (padded_end * m::SAMPLE_RATE as f64) as usize;

        let pcm_segment = if start_sample < pcm_data.len() {
            let end = std::cmp::min(end_sample, pcm_data.len());
            &pcm_data[start_sample..end]
        } else {
            continue;
        };

        if pcm_segment.is_empty() {
            continue;
        }
        ...
        for seg in &mut res {
            ...
            info!("seg.duration: {}", seg.duration);
    ...
    for segment in segments {
        ...
        if !segment.sub_segments.is_empty() {
            for sub in &segment.sub_segments {
                // 1. 写入序号
                text_to_str.push(format!("{}", counter));
...
pub async fn run_whisper_asr(input: &str, lang: Option<&str>) -> Result<()> {
    ...
    info!("正在处理音频[{:?}]: {:?}", lang.unwrap_or(&"en"), input);
    let result = run_whisper(ASR_MODEL_DIR, input, &device, lang);
    ...
    match result {
        Ok(segments) => {
            ...
            let _ = generate_srt(&segments, output_path.as_str()).await;
        }
        Err(e) => {
