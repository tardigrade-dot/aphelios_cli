...
use candle_nn::{linear, linear_no_bias, Linear, RmsNorm};
...
impl Attention {
    fn new(rotary_emb: Arc<RotaryEmbedding>, cfg: &Config, vb: VarBuilder) -> Result<Self> {
        ...
        let q_proj = candle_nn::linear_no_bias(hidden_sz, num_heads * head_dim, vb.pp("q_proj"))?;
        let k_proj =
            candle_nn::linear_no_bias(hidden_sz, num_kv_heads * head_dim, vb.pp("k_proj"))?;
        let v_proj =
            candle_nn::linear_no_bias(hidden_sz, num_kv_heads * head_dim, vb.pp("v_proj"))?;
        let o_proj = candle_nn::linear_no_bias(num_heads * head_dim, hidden_sz, vb.pp("o_proj"))?;

        // QK Norm (if present in weights)
    ...
    fn forward(
        ...
        let mut key_states = key_states.reshape((b_sz, q_len, self.num_kv_heads, self.head_dim))?;

        if let Some(norm) = &self.q_norm {
            query_states = norm.forward(&query_states)?;
        }
        if let Some(norm) = &self.k_norm {
            key_states = norm.forward(&key_states)?;
        }
        ...
        let attn_output = {
            ...
            attn_weights.matmul(&value_states.contiguous()?)?
        };
        attn_output
            .transpose(1, 2)?
            .contiguous()?
...
impl ModelForCausalLM {
    ...
    pub fn embed_tokens(&self, input_ids: &Tensor) -> Result<Tensor> {
        self.base_model.embed_tokens(input_ids)
    }
    ...
    pub fn forward_with_embeds(
        ...
        let (_b_size, seq_len, _) = inputs_embeds.dims3()?;
        self.base_model
            .forward_with_embeds(inputs_embeds, seqlen_offset, None)?
            .narrow(1, seq_len - 1, 1)?
            .contiguous()?
            .apply(&self.lm_head)
    }
