...
pub struct StreamingOptions {
    ...
    /// Maximum audio history to keep in seconds (default: 60.0).
    /// Older audio is trimmed to prevent unbounded memory growth during
    /// long-running microphone sessions.
...
impl StreamingOptions {
    ...
    /// Set the maximum audio history to keep (in seconds).
    /// Older audio is trimmed to prevent unbounded memory growth.
    pub fn with_max_audio_secs(mut self, secs: f32) -> Self {
...
impl StreamingState {
    ...
    ///
    /// Clears audio buffers, token history, and encoder cache without
    /// requiring a new `init_streaming()` call. Language and chunk options
    /// are preserved.
    pub fn reset(&mut self) {
        self.buffer.clear();
        self.audio_accum.clear();
        self.chunk_id = 0;
        self.raw_token_ids.clear();
        self.language.clear();
        self.text.clear();
...
/// Also trims `audio_accum` to stay within `max_audio_secs` to prevent
/// unbounded memory growth during long microphone sessions.
fn try_drain_chunk(state: &mut StreamingState) -> bool {
    ...
    if state.audio_accum.len() > max_samples {
        let trim = state.audio_accum.len() - max_samples;
        state.audio_accum.drain(..trim);
    }
