...
fn run_ocr_ui() -> Result<()> {
    ...
    std::thread::spawn(move || {
        // blocking_recv() 会阻塞当前后台线程直到收到消息
        while let Ok(msg) = log_rx.blocking_recv() {
            // 使用 upgrade_in_event_loop 安全地在 UI 线程中更新
        ...
        move || {
            ...
            std::thread::spawn(move || {
                ...
                let result = match DolphinModel::load_model(&model_path) {
                    Ok(mut dolphin_model) => {
                        let rt = tokio::runtime::Builder::new_current_thread()
                            .enable_all()
                            .build()
                            .unwrap();
                        rt.block_on(dolphin_model.dolphin_ocr(&input_file, &output_dir))
                    }
                    Err(e) => {
    ...
    window.on_start_asr({
        ...
        move || {
            ...
            std::thread::spawn(move || {
                ...
                //     let (_, samples_f32) = core_utils::normalize_audio(&audio_file, 16000)?;
                //     Ok(parakeet
                //         .transcribe_samples(samples_f32, 16000, 1, Some(TimestampMode::Sentences))?
                //         .text)
                // })();
