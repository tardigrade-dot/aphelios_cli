...
const DEFAULT_MODEL_ID: &str = "/Volumes/sw/pretrained_models/PaddleOCR-VL";
...
pub fn run_paddleocr(image: Vec<String>) -> Result<()> {
    let max_length = 1024;
    let task = Task::Ocr;
    ...
    if is_batch {
        ...
        for (idx, image_path) in batch.iter().enumerate() {
            println!("[{}/{}] Processing: {}", idx + 1, batch.len(), image_path);
    ...
    if is_multi_image {
        ...
        for (idx, image_path) in image.iter().enumerate() {
            println!(
                "[Page {}/{}] Processing: {}",
                idx + 1,
                image.len(),
        ...
        println!("{:=<60}", "");
        println!("Combined {} Output ({} pages):", task.prompt(), image.len());
        println!("{:=<60}", "");
    ...
    println!("Processing image: {}", image[0]);
    let (pixel_values, grid_thw) = load_image(&image[0], &device, dtype)?;
